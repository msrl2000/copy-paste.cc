...
interface TargetTileProps extends React.HTMLProps<HTMLDivElement> {
  client: ClientModel;
  variant?: 'small' | 'medium' | 'big' | 'bignoname' ;
  showClientName?: boolean;
}

interface NetworkTileProps {
  client: ClientModel;  // <-- Here
    ...
    ({ client, className, variant = 'small', children, ...props }, ref) => {
      ...
      return (
        <div
          ...
          {...props}
        >
          {(variant === 'big' || variant === 'bignoname') && displayIcons && (
          ...
          {children}
		   {variant !== 'bignoname' && <div className={styles.name}>{client.clientName}</div>}
        </div>
      );
    }
  )
